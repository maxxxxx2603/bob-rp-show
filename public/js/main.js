...
function buildEditor(){
  return `<div style="display:flex;height:540px;margin:-20px -22px">
    <div style="width:170px;background:var(--bg1);border-right:.5px solid var(--gb);padding:10px 8px;display:flex;flex-direction:column;gap:2px;flex-shrink:0">
      ${[['design','ti-palette','Design'],['channels','ti-hash','Channels'],['roles','ti-shield','Rôles'],['grades','ti-award','Grades & Paie'],['articles','ti-package','Articles'],['cv','ti-file-text','Questions CV'],['token','ti-lock','Token (Admin)']]
    ...
    <div style="flex:1;display:flex;flex-direction:column;overflow:hidden">
      ${pDesign()}${pChannels()}${pRoles()}${pGrades()}${pArticles()}${pCV()}${pToken()}
...
function pDiscord(){return ''}
function pToken(){return `<div class="cfg-panel" id="cp-token">
  <div style="background:rgba(239,68,68,0.08);border:.5px solid rgba(239,68,68,0.25);border-radius:10px;padding:12px 14px;margin-bottom:14px;font-size:12px;color:#f87171;display:flex;align-items:center;gap:8px">
    <i class="ti ti-shield-lock" style="font-size:16px;flex-shrink:0"></i>
    <span>Zone réservée au gestionnaire. Le token Discord ne doit jamais être partagé.</span>
  </div>
